...
try {
		...
		const userToUpdate = await User.findById(req.params.uid);
		...
		const updatedUser = await User.findByIdAndUpdate(req.params.uid, { role: newRole }, { new: true, runValidators: true });
