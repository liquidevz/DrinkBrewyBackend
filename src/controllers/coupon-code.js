...
const createCouponCodeForAdmin=async(req,res)=> {
    try {
      ...
      if (!data) {
        return res.status(404).json({ success: false, message: 'CouponCode Not Found' });
      }
 ...
 const updatedCouponCodeForAdmin=async(req,res) =>{
    try {
      ...
      const updatedCouponCode = await CouponCode.findOneAndUpdate({ _id: id }, { ...data }, { new: true });
      if (!updatedCouponCode) {
        return res.status(404).json({ success: false, message: 'CouponCode Not Found' });
      }
      return res.status(201).json({ success: true, data: updatedCouponCode });
    } catch (error) {
      return res.status(400).json({ success: false, message: error.message });
    }
  }
